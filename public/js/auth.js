$(document).ready(function(){
    ...
    schools.map(function(obj){
        loadedschools.push(obj.name);
    });

    function closeDropDown(id){
        
        $("#"+id+" + .dropdownwrap .dropdown").css("height", "0px");
        $('#university span.svghold').css({'display':'none'});
        setTimeout(function(){
            $("#"+id+" + .dropdownwrap").css("display", "none");
            $("#"+id).attr("openstate", 0);
        }, 300);
    }
    function load_dditem(){
        let apstr = '';
        loadedschools.forEach(el => {
            apstr += '<div class="dditem">'+el+'</div>';
        });
        $('#institution .dropdownwrap .dropdown').html(apstr);
    ...
    $(".ddhold").click(function(){
        ...
        load_dditem();
        $('#university span.svghold').css({'display':'block'});
    ...
    $(".orderbybutTab .orderbybut").click(function(){

        let mchecked = $(this).attr("mychecked");

        $(".orderbybutTab .orderbybut").attr("mychecked", 0).css("font-weight", "normal");
        $(".orderbybutTab .orderbybut").find(".obcircle").css("background-color", "white");
        if (mchecked == 0){
            $(this).attr("mychecked", 1).css("font-weight", "bold");
            $(this).find(".obcircle").css("background-color", "green");
        }
        if (mchecked == 1){
            $(this).attr("mychecked", 1).css("font-weight", "bold");
            $(this).find(".obcircle").css("background-color", "green");
        ...
        ordval = $(this).attr("ref");
    });

    $('span#schoolfilter').click(function(){
        let stext = $('input#schoolsearch').val();
        loadedschools = [];
    ...
    $('#submit').click(function(){
        if (submit_clickable){      
            ...
            $(".orderbybutTab .orderbybut").each(function(){
                if ($(this).attr('mychecked') == 1){
                    coldata['gender'] = $(this).attr('gend')
                }
            })
